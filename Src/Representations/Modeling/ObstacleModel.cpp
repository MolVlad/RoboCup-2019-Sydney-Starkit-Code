...
//  std::sort(const_cast<std::vector<Obstacle, Eigen::aligned_allocator<Obstacle> >&>(obstacles).begin(), const_cast<std::vector<Obstacle, Eigen::aligned_allocator<Obstacle> >&>(obstacles).end(), [](const Obstacle& a, const Obstacle& b) {return a.center.squaredNorm() < b.center.squaredNorm(); });
//VINCENZO
