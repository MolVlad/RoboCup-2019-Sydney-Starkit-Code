...
bool Motion::main()
{
  ...
  /*
  zittito cognition, e piallato naoprovider send crasha qui, probabilmente in waitforframedata
  */
  if(Blackboard::getInstance().exists("JointSensorData")){
    ...
    Thread::sleep(10);
    (_lastcicletick  = Time::getCurrentSystemTime() - _lastcicletick) < 11 ? Thread::sleep(_lastcicletick) : Thread::sleep(0) ;
  }
