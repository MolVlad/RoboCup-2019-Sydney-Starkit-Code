...
void projectWithoutUsingSSE(float img[2], float p[4], float p0[4], float p1[4], float p2[4])
{
	__m128 nom;
	__m128 denom;
	alignas(16) float temp0[4];
	alignas(16) float temp1[4];
	alignas(16) float temp2[2];
	for(int i = 0; i < 4; ++i){
		temp0[i] = p[i] * p0[i];
		temp1[i] = p[i] * p1[i];
	}
	nom = _mm_hadd_ps(*((__m128*)temp0), *((__m128*)temp1));
	...
	denom = _mm_hadd_ps(denom, denom);
  	denom = _mm_hadd_ps(denom, denom);
	_mm_storel_pi((__m64*) temp2, _mm_div_ps(nom, denom));
	img[0] = temp2[0];
	img[1] = temp2[1];
}
