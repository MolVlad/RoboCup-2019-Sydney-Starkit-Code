...
NaoCamera::NaoCamera(const char* device, CameraInfo::Camera camera, int width, int height, bool flip,
  ...
  HEIGHT(height)
{ 
  this->fd = open(device, O_RDWR | O_NONBLOCK, 0);
      //std::cerr << this->fd;
  VERIFY((this->fd) != -1);
  setImageFormat();
  mapBuffers();
...
void NaoCamera::setSpecialSettings(){
  
  // Buffer uvc_xu for control query HFLIP
    __u8 myBuff = (__u8) this->specialSettings.horizontalFlip.value;
    uvc_xu_control_query query_hflip;
    query_hflip.query = UVC_SET_CUR;
    query_hflip.data = &myBuff;
    // Unit always must be 3
    query_hflip.unit = 3;
    // Selector check  BUMAN document
    query_hflip.selector = 12;
    query_hflip.size = 2;
    VERIFY(ioctl(this->fd, UVCIOC_CTRL_QUERY, &query_hflip) != -1);
  
    // Buffer uvc_xu for control query VFLIP
    __u8 myBuff_v = (__u8) this->specialSettings.verticalFlip.value;
    uvc_xu_control_query query_vflip;
    query_vflip.unit = 3;
    query_vflip.selector = 13;
    query_vflip.size = 2;
    query_vflip.query = UVC_SET_CUR;
    query_vflip.data = &myBuff_v;
    VERIFY(ioctl(this->fd, UVCIOC_CTRL_QUERY, &query_vflip) != -1);
...
bool NaoCamera::captureNew(NaoCamera& cam1, NaoCamera& cam2, int timeout)
{
  ...
  if(polled < 0)
  {
    ...
    FAIL("Cannot poll for camera images. Reason: " << strerror(errno) << ".");  //TODO VINCENZO
    return false;
  }
  else if(polled == 0)
  {
  ...
  for(int i = 0; i < 2; ++i)
  {
    if(pollfds[i].revents & POLLIN)
    {
      ...
      if(errno != EAGAIN)
      {
       OUTPUT_ERROR("VIDIOC_DQBUF failed: " << strerror(errno));  //TODO VINCENZO
        return false;
      }
      else
      {
...
bool NaoCamera::captureNew()
{
  ...
  if(polled < 0)
  {
    OUTPUT_ERROR(TypeRegistry::getEnumName(camera) << "camera : Cannot poll. Reason: " << strerror(errno));  //TODO VINCENZO
    FAIL(TypeRegistry::getEnumName(camera) << "camera : Cannot poll. Reason: " << strerror(errno) << ".");   //TODO VINCENZO
  }
  else if(polled == 0)
  {
    OUTPUT_ERROR(TypeRegistry::getEnumName(camera) << "camera : 200 ms passed and there's still no image to read from the camera. Terminating.");  //TODO VINCENZO
    return false;
  }
  else if(pollfd.revents & (POLLERR | POLLNVAL))
  {
    OUTPUT_ERROR(TypeRegistry::getEnumName(camera) << "camera : Polling failed.");   //TODO VINCENZO
    return false;
  }
...
Matrix5uc NaoCamera::getAutoExposureWeightTable() const
{
  ...
  for(int y = 0; y < 4; ++y)
  {
    for(int x = 0; x < 4; ++x)
    {
      table(y, x) = static_cast<uint8_t>(appliedSettings.autoExposureWeightTable[4 * y + x].value);
    }
  }
  return table;
}
...
void NaoCamera::setSettings(const CameraSettings::CameraSettingsCollection& cameraSettingCollection, const Matrix5uc& autoExposureWeightTable)
{
  ...
  for(int y = 0; y < 4; ++y)
  {
    for(int x = 0; x < 4; ++x)
    {
      settings.autoExposureWeightTable[4 * y + x].value = autoExposureWeightTable(y, x);
      settings.autoExposureWeightTable[4 * y + x].enforceBounds();
    }
  }
}
...
currentTableEntry.value=0;
appliedTableEntry.value=0;
...
void NaoCamera::readCameraSettings()
{
  ...
  //  getControlSetting(setting);   // TODO UNCOMMENT VINCENZO
}
...
void NaoCamera::doAutoWhiteBalance()
{
  ...
  if(getControlSetting(settings.settings[CameraSettings::white_balance_temperature]))
  {
    OUTPUT_TEXT("New white balance is " << settings.settings[CameraSettings::white_balance_temperature].value);
    appliedSettings.settings[CameraSettings::white_balance_temperature] = settings.settings[CameraSettings::white_balance_temperature];
  }*/
}
...
void NaoCamera::checkV4L2Setting(V4L2Setting& setting) const
{
  ...
  {
    FAIL("ioctl to query setting failed for camera setting " << setting.command);   //TODO VINCENZO
  }
  if(queryctrl.flags & V4L2_CTRL_FLAG_DISABLED)
  {
    FAIL("Camera setting " << setting.command << " is disabled");     //TODO VINCENZO
  }
  if(queryctrl.type != V4L2_CTRL_TYPE_BOOLEAN && queryctrl.type != V4L2_CTRL_TYPE_INTEGER && queryctrl.type != V4L2_CTRL_TYPE_MENU)
  {
    FAIL("Camera setting " << setting.command <<  "is unsupported ");  //TODO VINCENZO
  }
...
bool NaoCamera::getControlSetting(V4L2Setting& setting)
{
  ...
  {
    OUTPUT_ERROR("NaoCamera: Retrieving camera setting " << setting.command << " failed");  //TODO VINCENZO
    return false;
  }
...
bool NaoCamera::setControlSetting(V4L2Setting& setting)
{
  ...
  if(ret < 0)
  {
    OUTPUT_ERROR("NaoCamera: Setting value ID: " << setting.command << " failed. VIDIOC_S_CTRL return value is " << ret);   //TODO VINCENZO
    return false;
  }
  return true;
}
...
bool NaoCamera::assertCameraSetting(CameraSettings::CameraSetting setting)
{
  ...
  return false;
//  return true;   //TODO REMOVE VINCENZO
...
{
  // set formatmemset(&fmt, 0, sizeof(v4l2_format));
  v4l2_format fmt;
...
NaoCamera::CameraSettingsCollection::CameraSettingsCollection()
{
  ...
  settings[CameraSettings::brightness] = V4L2Setting(V4L2_CID_BRIGHTNESS,255,-255,255);
  settings[CameraSettings::contrast] = V4L2Setting(V4L2_CID_CONTRAST, 32,0,255);
  settings[CameraSettings::saturation] = V4L2Setting(V4L2_CID_SATURATION,64,0,255);
  settings[CameraSettings::hue] = V4L2Setting(V4L2_CID_HUE,0,-180,180);
  settings[CameraSettings::gain] = V4L2Setting(V4L2_CID_GAIN, 16, 0, 1023);
  settings[CameraSettings::sharpness] =V4L2Setting(V4L2_CID_SHARPNESS, 4, 0, 9);
  settings[CameraSettings::whiteBalanceTemperature] = V4L2Setting(V4L2_CID_WHITE_BALANCE_TEMPERATURE, 0, 2500, 6500);
  settings[CameraSettings::focusAbsolute] = V4L2Setting(V4L2_CID_FOCUS_ABSOLUTE, 250, 0, 250);
  settings[CameraSettings::exposureAbsolute] = V4L2Setting(V4L2_CID_EXPOSURE_ABSOLUTE, 512, 0, 1048575);
  settings[CameraSettings::autoFocus] = V4L2Setting(V4L2_CID_FOCUS_AUTO, 0, 0, 1, CameraSettings::autoFocus);
  settings[CameraSettings::autoExposure] = V4L2Setting(V4L2_CID_EXPOSURE_AUTO, 0, 0, 1, CameraSettings::autoExposure);
  settings[CameraSettings::autoHue] = V4L2Setting(V4L2_CID_HUE_AUTO, 0, 0, 1, CameraSettings::autoHue);
  settings[CameraSettings::autoWhiteBalance] = V4L2Setting(V4L2_CID_AUTO_WHITE_BALANCE, 1, 0, 1, CameraSettings::autoWhiteBalance);
}
...
//  doAutoWhiteBallance(V4L2_CID_DO_WHITE_BALANCE, 1, 1, 1)  //TODO FIX VINCENZO
{}